    ...
    def build(self, sharedlib_only=False, model_only=False):
        if not model_only:
            build.case_build(self._caseroot, case=self._case,
                             sharedlib_only=True, model_only=False)
        ...
        if not sharedlib_only:
            ...
            run_cmd(cmake_cmd, from_dir=exeroot)
            run_cmd("make -j8 >>& homme.bldlog", from_dir=exeroot)

            post_build(self._case, logs)
    ...
    def run(self):
        try:
            ...
            if generate:
                ...
                run_cmd("make -j 4 baseline >& %s" % log, from_dir=exeroot)
                ...
                run_cmd("make -j 4 check >& %s" % log, from_dir=exeroot)
            else:
                run_cmd("make -j 4 baseline >& %s" % log, from_dir=exeroot)
